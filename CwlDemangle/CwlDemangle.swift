...
func archetypeName(_ index: UInt32, _ depth: UInt32) -> String {
	...
	var i = index
	repeat {
		result.append(String(describing: UnicodeScalar(UnicodeScalar("A").value + i % 26)))
		i /= 26
	} while i > 0
	if depth != 0 {
...
public struct ScalarScanner<C: Collection, T> where C.Iterator.Element == UnicodeScalar, C.Index: Comparable {
	...
	public mutating func readWhileTrue(_ f: (UnicodeScalar) -> Bool) -> String {
		var string = ""
	...
	public mutating func skipWhileTrue(_ f: (UnicodeScalar) -> Bool) {
		while index != scalars.endIndex {
			if !f(scalars[index]) {
				return
			}
...
extension TextOutputStream {
	mutating func write<S: Sequence, T: Sequence>(_ sequence: S, labels: T, render: (inout Self, S.Iterator.Element) -> ()) where T.Iterator.Element == String? {
	...
	mutating func write<S: Sequence>(_ sequence: S, prefix: String? = nil, separator: String? = nil, suffix: String? = nil, render: (inout Self, S.Iterator.Element) -> ()) {
		if let p = prefix {
			write(p)
		}
		var first = true
...
public struct SwiftName: CustomStringConvertible {
	...
	func printEntity<T: TextOutputStream>(_ output: inout T, extraName: String, options: PrintOptions) {
	...
	func print<T: TextOutputStream>(_ output: inout T, _ options: PrintOptions = PrintOptions()) {
		switch kind {
		case .static:
